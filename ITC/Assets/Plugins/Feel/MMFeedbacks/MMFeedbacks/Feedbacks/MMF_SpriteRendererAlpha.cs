...
namespace MoreMountains.Feedbacks
{
	...
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("此反馈可让你随时间修改目标 SpriteRenderer 的透明度。")]
	...
	public class MMF_SpriteRendererAlpha : MMF_Feedback
	{
		...
		[Tooltip("作用模式：`Instant` 立即设置透明度；`OverTime` 按 `AlphaOverTime` 变化；目标模式使用 `ToDestinationAlpha` 与曲线。")]
		public Modes Mode = Modes.OverTime;
		/// how long the sprite renderer should change over time
		[Tooltip("在非 `Instant` 模式下的透明度变化时长（秒）。")]
		...
		[Tooltip("在 `OverTime` 模式下用于驱动透明度变化的曲线。")]
		...
		[Tooltip("在目标透明度模式下使用的插值曲线；`ToDestinationAlphaAndBack` 会按同曲线往返。")]
		[MMFEnumCondition("Mode", (int)Modes.ToDestinationAlpha, (int)Modes.ToDestinationAlphaAndBack)]
