...
{    
	...
	[FeedbackHelp("此反馈可通过 MMSoundManager 播放声音。要生效，场景中必须存在 MMSoundManager。你可以使用 AudioClip / AudioResource、随机策略、淡入淡出、Solo 与 3D 音频参数；若指定 SoundDataSO，会覆盖本反馈中的大多数同名设置（即本反馈同名字段优先级更低）。")]
	public class MMF_MMSoundManagerSound : MMF_Feedback
	{
		...
		public override string RequiresSetupText { get { return "此反馈至少需要在下方 Sfx 指定一个 AudioClip，或在 RandomSfx 数组中提供一个或多个 AudioClip。若使用 Source=AudioResource，请确保已填写 AudioResourceToPlay。"; } }
		#endif
		...
		[Tooltip("选择播放来源：AudioClip 或 AudioResource。该选择会决定下方哪组字段生效。")]
		...
		[Tooltip("可选的 AudioResource。使用该来源时会忽略上方 Sfx。")]
		...
		[Tooltip("用于随机挑选音效片段的 AudioClip 数组。若非空，将覆盖上方单个 Sfx。")]
		...
		[Tooltip("任选：MMF_MMSoundManager声音数据。指定后会覆盖本反馈中大多数同名设置（含 Sfx/随机特效/音量/音高/轨道/淡变/独奏/3D 等）；本反馈对应同名字段优先级别。")]
		...
		[Tooltip("停止此反馈时是否同时停止该声音。若开启 FadeOutOnStop，会先淡出再停止。")]
		...
		[Tooltip("声音跟随的 Transform。留空则按触发位置播放。若 SoundDataSO 中也设置了该值，空值时会采用 SO 的值。")]
